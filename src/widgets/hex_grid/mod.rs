...
impl<'a, F> HexGrid<'a, F> {
	/// Create a button context to be built upon.
	pub fn new() -> HexGrid<'a, F> {
		HexGrid {
...
impl<'a, F> Widget for HexGrid<'a, F>
	where F: FnMut()
{
	...
	fn unique_kind(&self) -> &'static str { "HexGrid" }
	...
	fn update<C: CharacterCache>(mut self, args: UpdateArgs<Self, C>) {
		...
		{
			// Recall that our HexGrid struct includes maybe_react, which
	...
	fn draw<C: CharacterCache>(args: DrawArgs<Self, C>) -> Element {
		...
		let (xy, dims) = rect.xy_dim();

		// Retrieve the styling for the Element.
		let color = state.color(style.color(theme));

		// Generate the grid form.
		let pressable_form = grid_form::gen(0.0, "".to_string(), color, &dims);
		...
		let maybe_label_form = None;
			...
			// 		.shift(xy[0].floor(), xy[1].floor() + (dim[1] / 2.0) - 15.0)
			// });
...
impl<'a, F> Colorable for HexGrid<'a, F> {
...
impl<'a, F> Labelable<'a> for HexGrid<'a, F> {
...
/// Represents the unique styling for our HexGrid widget.
...
/// Represents the unique, cached state for our HexGrid widget.
