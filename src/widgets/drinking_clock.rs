...
impl<'a, F> DrinkingClock<'a, F> {
	/// Create a button context to be built upon.
	pub fn new() -> DrinkingClock<'a, F> {
		DrinkingClock {
	...
	/// Sets orientation (rotation) of the widget.
	#[allow(dead_code)]
	pub fn whatever_number(mut self, number: f64) -> Self {
		self.whatever_number = number;
		self
	}
}
...
impl<'a, F> Widget for DrinkingClock<'a, F>
	where F: FnMut()
{
	...
	fn unique_kind(&self) -> &'static str { "DrinkingClock" }
	...
	fn update<C: CharacterCache>(mut self, args: UpdateArgs<Self, C>) {
		...
		{
			// Recall that our DrinkingClock struct includes maybe_react, which
		...
		if state.view().whatever_number != self.whatever_number {
			state.update(|state| {
				state.whatever_number = self.whatever_number;
			})
		}
	}
	...
	fn draw<C: CharacterCache>(args: DrawArgs<Self, C>) -> Element {
		...
		let pressable_form: elmesque::Form = drinking_clock(state.whatever_number,
			state.maybe_label.as_ref().map(|label_text| label_text.to_string())
			.unwrap_or("Crazyness".to_string()));
		...
		let maybe_label_form: Option<elmesque::Form> =
			...
			state.maybe_label.as_ref().map(|label_text| {
				...
				form::text(Text::from_string("What time is it?".to_string())
					.color(label_color).height(size as f64))
					.shift(xy[0].floor(), xy[1].floor() + (dim[1] / 2.0) - 20.0)
			});
...
impl<'a, F> Colorable for DrinkingClock<'a, F> {
...
impl<'a, F> Labelable<'a> for DrinkingClock<'a, F> {
...
/// Represents the unique styling for our DrinkingClock widget.
...
/// Represents the unique, cached state for our DrinkingClock widget.
...
#[allow(dead_code)]
pub fn drinking_clock(secs: f64, label: String) -> Form {
	use elmesque::color::{ self, blue, dark_blue, light_blue, dark_purple, white };
